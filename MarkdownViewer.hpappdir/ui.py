...
# ---------------------------------------------------------------------------
# Go-to-percent dialog
# ---------------------------------------------------------------------------

def show_goto_dialog():
    """Show PPL INPUT dialog to jump to a document percentage.

    Returns 0-100 integer, or None on cancel.
    """
    from hpprime import eval as heval
    heval('P:=50')
    ok = heval('INPUT(P,"Go to %","Position (0-100):")')
    if ok:
        p = int(heval('P'))
...
def show_shortcuts_overlay(colors=None):
    """Show all keyboard shortcuts in a full-screen overlay.

    Blocks until user presses a key or taps.
    """
    c = _c(colors)
    dx, dy = 10, 8
    dw, dh = 300, 210
    ...
    tw = text_width(title, FONT_14)
    draw_text(GR_AFF, dx + (dw - tw) // 2, dy + 4, title, FONT_14,
              c.get('header', c['ctx_text']))
    draw_rectangle(GR_AFF, dx + 10, dy + 22, dx + dw - 10, dy + 23,
                   c['ctx_border'], 255, c['ctx_border'], 255)
    shortcuts = [
        ...
        ("LOG", "Jump to end"),
        ("Left / Right", "Back / Forward file"),
        ("ESC", "Go back / Exit"),
        ("Drag", "Touch scroll"),
        ("Long press", "Context menu"),
        ("Tap header", "Collapse / expand"),
        ("Tap link", "Open linked .md file"),
        ("Scrollbar", "Tap or drag to seek"),
    ]
    y = dy + 28
    hdr_c = c.get('header', c['ctx_text'])
    for key, desc in shortcuts:
        draw_text(GR_AFF, dx + 15, y, key, FONT_10, hdr_c, 120)
        draw_text(GR_AFF, dx + 140, y, desc, FONT_10, c['ctx_text'], 155)
        y += 15
    # Hint
